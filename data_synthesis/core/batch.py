...
def run_batch(
    ...
    Returns:
        若因单条 pipeline 异常或未捕获错误中止则为 False；超时、达上限或正常结束为 True。
    """
    ...
    try:
        for task_provider in batch_provider.iter_task_providers():
            ...
            try:
                ...
                executed_total += 1
                failure_count += 1
                stop_reason = "pipeline_exception"
                last_pipeline_error = f"{type(exc).__name__}: {exc}"
                batch_ok = False
                print(
                    "  [batch] 本条 pipeline 抛出异常，中止批量任务: "
                    f"{last_pipeline_error}"
                )
                break
        ...
        elif stop_reason == "pipeline_exception":
            reason_msg = "某条 pipeline 抛出异常，已中止后续任务"
        elif stop_reason == "pipeline_failed":
            reason_msg = "某条 pipeline 返回失败，已中止后续任务"
        else:
        ...
        if last_pipeline_error is not None:
            print(f"  最后 pipeline 异常: {last_pipeline_error}")
