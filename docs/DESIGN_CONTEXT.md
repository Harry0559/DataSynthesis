...
- 实现：`CursorTabLogCollector`（快捷键保存 Tab 日志 + 读日志写 jsonl）（TODO）。
...
- 具体实现（PlanFileProvider、GitRepoProvider、CursorAdapter、CursorTabLogCollector 等）由 `__main__` 组装后注入；**箭头只向下，无环**。
...
| 采集 | Collector | （基类） | CursorTabLogCollector |
...
- **collectors/cursor_tab_log.py**：init_session、collect（发快捷键、等日志、读文件、写 jsonl）、finalize。
