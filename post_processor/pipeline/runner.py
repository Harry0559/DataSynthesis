...
def run_postprocessor(config: PipelineConfig) -> RunStats:
    ...
    input_type = (
        RAW
        if input_source.needs_integration
    ...
    output_format = validate_pipeline(step_instances, input_type)
    ...
    for item in input_source.iter_items():
        ...
        step_idx = 0
        if input_source.needs_integration:
            sample = stream_steps[0][1].process(sample)
            if sample is None:
        ...
        for i in range(step_idx, len(stream_steps)):
            ...
            if step_type == FILTER:
                sample = step_instance.process(sample)
                if sample is None:
    ...
    for deduper in dedup_steps:
        buffer = deduper.deduplicate(buffer)
